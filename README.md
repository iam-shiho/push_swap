...
* **解説と空間・時間計算量**:
  ...
  `操作数を減らすために`
ｂに送る際に数を数えてその数が偶数だった場合は上に奇数だった場合はraをして下に入れる　最上位にいち早く持っていけるように
...
それを座標圧縮の最大値のbit数分繰り返す
 `操作数を減らすために`
座標圧縮することによって最大値がノードの総数となり、比べるbitの数が減る
...
```bash
$ ARG="4 67 3 87 23"; ./push_swap --bench $ARG 2> bench.txt \vert{} ./checker$ARG //エラー出力をbench.txtに出力
OK
$ cat bench.txt
